...
class PlayerInstance {
    ...
    changeIndexDependLoop (style) { // 根据循环模式切换索引
        switch ( this.loopMode ) {
            case 0: 
                ...
                break;
            case 1:
                this.songIndex = this.randomIndex()
                break;
            case 2:
    ...
    randomIndex () {
        ...
        randomIndex === this.songIndex ? randomIndex=this.randomIndex() : randomIndex=randomIndex
        return randomIndex;
    }
    // 切换歌曲 
    // params style || index
    ...
    turnMute () {
        ...
        // 更改静音icon
        if(this.el.muted) {
            this.mute.el.className=('iconfont icon-muted')
            return;
        }
        this.mute.el.className=('iconfont icon-volume')
    }
    // 改变循环模式
    turnLoop() {
        this.loopMode === 2 ? this.loopMode = 0 : this.loopMode += 1;
        switch (this.loopMode){
            case 0:
            this.loop.el.className=('iconfont icon-loop player-ui__btn--loop text-primary')
                break;
            case 1:
            this.loop.el.className=('iconfont icon-random player-ui__btn--loop text-primary')
                break;
            case 2:
            this.loop.el.className=('iconfont icon-single player-ui__btn--loop text-primary')
                break;
            default: break;
        }
    }
    ...
    bindEvents () { // 绑定事件的方法
        ...
        // 点击音量按钮切换是否静音
        this.mute = new PlayerBtn('#btn-muted',{
            'click':this.turnMute.bind(this)
        })
        this.loop = new PlayerBtn('#btn-loop',{
            'click':this.turnLoop.bind(this)
        })
        // 进度条
        this.$el.on('canplay', () => {
            // 更改总时长显示
